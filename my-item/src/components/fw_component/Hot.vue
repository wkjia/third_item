...
export default {
  data: function() {
    return {
      //获取sessionStorage里面的option
      datum: JSON.parse(sessionStorage.getItem("option")),
      number:sessionStorage.getItem("value"),
      //页面渲染
      total: [],
      page: "",
      num: 0,
  ...
  methods: {
    ...
    query() {
      ...
      $.ajax({
        ...
        success(data) {
          ...
          console.log(data6);
          var shuju = self.datum.data.items;
          self.total =shuju != "" ? shuju : self.total.concat(data6.data.items);
    ...
    snum(){
        this.number = sessionStorage.getItem("value")
    }
  },
  watch: {
    number: {
      deep: true,
      handler() {
        this.total = JSON.parse(sessionStorage.getItem("option")).data.items
        this.num = 0;
  ...
  mounted() {
    this.page = sessionStorage.getItem("value");
    this.query();
